...
/**
 * Check if the given data is valid considered the schema
 * Use https://github.com/ajv-validator/ajv to ensure that
 * the check is fully compliant to the JSON Schema specification.
 * @param {object} schema global classes references in { definitions: ... }
 * @param {object} template schema for the current object
 * @param {object} data object to check versus the template
 * @returns true if valide, false otherwise
 */
 function isValid(schema, template, data) {
  console.log({schema})
  const ajv = new Ajv({ strictSchema: false });
  const validate = ajv.addSchema(schema).compile(template);
...
function findRequirements(config, template) {
  ...
  console.log()
  if (!isValid({ definitions: template }, root[1], config)) {
    throw new Error("The config is not compliant with the JSON Schema template");
  }
