 ...
 * @param {*} force flag to force the addition of childs of a required field in the dt
 * @returns
 */
function dfs(propertyId, template, config, list, force = false) {
  ...
  if (objInTemplate.required) {
    const hasRequiredName = objInTemplate.required.find((r) => {
  ...
  for (const [k, v] of Object.entries(valueC)) {
    ...
    if (templateRepresentation && isNestedObject(templateRepresentation)) {
      ...
      // and in the same level of a required field
      // or a child of a required field
      // and the key contains name
...
function findRequirements(config, template) {
  ...
  if (!root) {
    return { error: `Could not find ${config.kind}` };
  }
 ...
 * @param {string} origin current id of the site
 * @param {number} index current position in the path
 * @return {number | undefined} the location
 */
function meetsRequirement(requirement, ressources, origin, index = 0) {
  ...
  if (index >= requirement.path.length) {
    if (requirement.value == ressources) {
      // check if it correspond
      return origin;
    }
    return undefined;
  }
  ...
  if (isArray(path)) {
    ...
    if (!ressources[path]) {
      return undefined; // the requirement was not in the ressources, we don't do anything
    }

    return meetsRequirement(
      requirement,
      ressources[path],
      ressources?.location ?? origin,
      index + 1
    );
  }
